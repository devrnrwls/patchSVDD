...
def search_NN(test_emb, train_emb_flat, NN=1, method='kdt'):
    ...
    for n in range(Ntest):
        for i in range(I):
            aa = test_emb[n, i, :, :]
            dists, inds = kdt.query(test_emb[n, i, :, :], return_distance=True, k=NN) #I=H=row, J=W=column
            closest_inds[n, i, :, :] = inds[:, :] #최근 접 이웃 NN개의 인덱스
            l2_maps[n, i, :, :] = dists[:, :] #가장 가까운 이웃 NN개까지의 거리
