...
def infer(x, enc, K, S):
    ...
    enc = enc.eval()
    with torch.no_grad():
        for xs, ns, iis, js in loader: #NCHW
            xs = xs.cuda()
            embedding = enc(xs)
...
def eval_embeddings_NN_multiK(obj, embs64, embs32, NN=1):
    ...
    maps_64 = distribute_scores(maps_64, (256, 256), K=64, S=16) #s = 256/64=4 -> 64/4 = 16 ??
    det_64, seg_64 = assess_anomaly_maps(obj, maps_64)

    emb_tr, emb_te = embs32
    maps_32 = measure_emb_NN(emb_te, emb_tr, method='ngt', NN=NN)
    maps_32 = distribute_scores(maps_32, (256, 256), K=32, S=4)  # s = 256/32=8 -> 32/8 = 4 ??
...
def measure_emb_NN(emb_te, emb_tr, method='kdt', NN=1):
    ...
    # anomaly_maps[anomaly_maps<2] =0 #sudopop 정상도 시뻘겋게 나오는게 보기 안좋아서
